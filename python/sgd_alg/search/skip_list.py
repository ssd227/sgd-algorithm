...
class SkipList:
    ...
    def log(self):
        ...
        for row in data:
            print(f"{str(row[0]).ljust(8)}{str(row[3]).ljust(8)}{str(row[4]).ljust(8)}{str(row[2]).ljust(7)}{str(row[1]).ljust(15)}")
        print('-------'*20)
    ...
    def get(self, key):
        ...
        # while p.level[i].next or i>0: # 注意:当i=0时，p.level[0] 一定存在值。 反例，真没有值，此时返回None也是合理的
    ...
    def delete(self, key):
        ...
        for i, pi in pre_map.items():
            ...
            if i < level_num:
                ...
                # 修正span
                if pi.level[i].span > 0: # 不用处理尾巴span
                    pi.level[i].span -= 1
        
        # todo 	又又又出大问题了，如果删除的是尾巴节点，则delete需要重塑造前置节点的next  
